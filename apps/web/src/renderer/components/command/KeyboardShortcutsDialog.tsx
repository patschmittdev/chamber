...
/**
 * Help overlay listing every command that carries a keybinding, grouped the same way
 * the palette groups its entries. Reachable via the `?` shortcut and from the
 * palette's "Keyboard shortcuts" command.
 */
export function KeyboardShortcutsDialog({ open, onOpenChange, commands }: KeyboardShortcutsDialogProps) {
  const groups = useMemo(
    () => groupCommands(commands.filter((command) => command.keybinding)),
    [commands],
  );
