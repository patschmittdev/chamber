...
interface Props {
  onSend: (message: string, attachments?: ChatAttachment[], metadata?: ComposerSendMetadata) => void;
  onStop: () => void;
  isStreaming: boolean;
  disabled: boolean;
...
export function ChatInput({ onSend, onStop, isStreaming, disabled, availableModels, selectedModel, onModelChange, placeholder, value, onValueChange, includeComposerMetadata = false }: Props) {
  ...
  const handleSubmit = useCallback(() => {
    ...
    const metadata = mentionTargets.length > 0 || (includeComposerMetadata && input.includes('@'))
      ? {
        mentionTargets,
    ...
    if (metadata) {
      onSend(input, sendAttachments, metadata);
    } else {
      onSend(input, sendAttachments);
    }
    resetComposer();
