...
- **Promote working memory to a first-class per-mind surface** - Working memory moves from a section buried inside the Overview tab to its own dedicated Memory tab on the agent detail card. The "View working memory" command (Cmd/Ctrl+Shift+M) now deep-links directly to the Memory tab for the active mind. Loading shows animated Skeleton placeholders, read errors appear as a destructive Alert, and an absent .working-memory directory shows an EmptyState instead of blank space. Each file card shows a human section label (Memory, Rules, Log) with the raw filename accessible via tooltip, displays the per-file last-modified time next to the section heading, and renders truncated files with a notice that includes the full file path as an open-full affordance. A refresh button lets the operator pull the latest files without leaving the panel and shows a last-fetched timestamp after each read. The fallback description when no precedence layer is available is reworded to match actual rendering behavior. Addresses findings rail-M1, rail-M2, rail-M3, rail-M4, rail-M5, and rail-M6.
- **Harden Canvas Lens actions and appearance** - Canvas Lenses now inherit Chamber appearance by default, support intentional fixed light or dark manifests, show trusted lifecycle status, and reload updated Lens content after actions.
- **Complete Lens refreshes reliably** - Subscribes before dispatching background prompts so delivered idle events are not missed, preventing completed Newspaper generations and Canvas header refreshes from waiting for the fallback.

- **Polish shell startup, tray controls, and low-risk responsiveness paths** - Deduplicates VoidScreen boot lines under StrictMode replay, upgrades the desktop tray with live agent/chatroom status plus quick stop/show actions, lazy-loads heavy activity-bar views through Suspense boundaries, makes open-existing mind activation optimistic while list hydration finishes in the background, adds elapsed reassurance for pending one-shot extensions intents, guards non-positive operator-activity timestamps from rendering nonsense relative times, and extends strict CSP coverage for packaged main-frame responses.
- **Improve history rail scannability with date grouping and clearer search feedback** - Conversation history now renders consistent section labels with counts (Pinned, Recent, Archived), groups rows by date headings, keeps long title truncation while exposing the full title on hover, and surfaces search result metadata with content-indexing progress when transcript matching is still loading.
- **Scale chatroom participant and render performance paths** - Adds a bounded participant bar with a visible +N overflow affordance and expand/collapse behavior, memoizes chatroom row derivation and moderator decision parsing, batches auto-scroll writes with animation frames, and reuses cached message-id lookup in the chatroom reducer to reduce per-delta O(N) pressure. Addresses findings perf-D2 and perf-D5.
- **Surface chat turn failures with an inline Retry** - A failed, timed-out, or empty chat turn now renders an inline destructive alert (role=alert) with a Retry action instead of dead-ending silently. The failure is derived once in the store reducer (per mind, single source of truth), set on error/timeout and on a non-cancelled completion that produced no assistant content, and cleared when a new turn starts or a turn completes with content. Retry reuses the existing regenerate path and the shared ui/Alert primitive. Addresses finding chat-F1.
- **Show Lens view toggle failures** - A failed Lens view enable/disable toggle now renders a visible inline error (reusing the shared extensions TabError) in addition to logging, matching the error affordance every sibling extensions tab already provides. Addresses finding ext-E7.
...
- **Unify keyboard-accessible row actions across the agents, history, and chat rows** - Adds shared Radix dropdown-menu, context-menu, and row-actions primitives and adopts one discoverability convention (persistent kebab overflow, right-click context menu, focus-within reveal, and tooltips) across the agents rail, conversation history rail, and chat message rows. Agents rail controls are now real keyboard-focusable buttons in an absolute overlay instead of nested button-role spans, which restores the mind name width and adds a filter input, and history rows keep pin inline while moving archive, export, rename, and delete into the kebab so they no longer overflow the panel edge. The message row's right-click Delete routes through the same two-step confirmation as the inline control, so a right-click can never truncate history without confirming. Addresses findings shell-F1, shell-F4, rail-H2, rail-H3, perf-D1, and shell-F9.
- **Polished composer and slash menu UX accessibility** - Added attachment chips with remove actions, textarea-hosted combobox ARIA wiring for slash and mention menus, grouped slash sections with icon cues, and clearer chat header plus system-prompt state indicators.

### Tests

- **Assert production wires the mind trust service.** Adds a security-boundary invariant that fails if `main.ts` stops injecting the trust service into MindManager and CronService, so a future mis-wire that silently disables the per-mind execution gate is caught at test time.
...
- **Add an app-wide notification host** - Introduces a decoupled toast queue in the store with a useToast() hook and a single Toaster host mounted once in AppShell that renders stacked notifications as ui/Alert banners with manual and auto dismiss. Any feature can enqueue a notification without a third-party toast dependency, and the host is not coupled to any specific surface. Addresses finding shell-F2 (remainder).
- **Add one-shot regenerate with a different model** - The assistant message action row keeps its plain Regenerate button (rerun with the current model) and, when more than one model is available, adds a compact caret menu beside it so a reply can be regenerated once with a chosen model without changing the mind's saved model. The menu reuses the existing popover and command primitives (searchable and keyboard accessible) and the composer's model-key mapping; renderer-only, with no contract change since chat.regenerate already accepts an optional model.
- **Add a user-authored prompt library** - Reusable prompt text now lives in a user-scoped library surfaced two ways: a new Prompts tab in Extensions creates, edits, and deletes prompts, and the composer slash menu lists saved prompts next to the built-in commands so selecting one inserts its body. Prompts persist to a single path-confined prompts.json under the config directory through an atomic store, and the new prompts namespace on the Electron contract exposes honest browser-mode capability entries.
- **Add conversation pin and archive** - The conversation history rail can now pin important conversations to the top and archive old ones into a collapsible "Archived" section, alongside the existing search, export, rename, and delete controls. Pin and archive state persists per conversation through the same store path as rename and does not disturb recency ordering. Extends the conversationHistory contract with setPinned and setArchived across the Electron host, with honest browser-mode capability manifest entries.
- **Add a per-conversation system prompt override** - The chat header now has a system-prompt control that shows whether the active conversation uses the agent default or a custom prompt, and lets you set or clear an override for just that conversation. An empty override falls back to the agent default. The override persists per conversation through the same store path as pin and archive without disturbing recency ordering, and is applied to the conversation's live session immediately when it is the active chat. Extends the conversationHistory contract with setSystemMessage across the Electron host, with an honest browser-mode capability manifest entry.
- **Add a command and keyboard shortcut registry** - Introduces a single typed registry that both the command palette and a new global keydown handler consume, so keybindings are declared once, checked for conflicts at build time, and surfaced in a Keyboard shortcuts help overlay reachable via ? and from the palette. Refactors buildCommandItems to consume the registry instead of a parallel hand-maintained list and migrates the existing Cmd/Ctrl+K handler onto it. Renderer-only platform foundation for the capability-parity initiative.
...
- **Add per-mind custom instruction precedence controls:** Adds per-mind opt-out of the global custom instructions (default inherit), sanitized instruction-precedence metadata surfaced in Settings, a pure prompt-composition helper that orders the global, per-mind, and Chamber safety layers, and per-conversation system-prompt snapshots so existing conversations keep their original instructions on resume, recover, and restore.
...
- **Make shell side rails symmetrically resizable** — Agents and Conversation History now keep independent persisted widths, expose keyboard-accessible resize separators, and use consistent 200ms collapse and expand motion while retaining responsive auto-collapse.
- **Task-oriented settings architecture** — Organizes settings into Workspace, Agents, Models and providers, and Sources and security with legacy deep-link aliases, source-directory handoffs, and bounded renderer-safe status messages.

### Security

- **Deny MCP servers to Canvas-action (no-tools) isolated sessions.** Canvas Lens actions run in an isolated prompt with an empty tool set, but the session still received the mind's trust-approved MCP servers, which could leak side-effect tool access into the untrusted canvas-action prompt. The no-tools canvas-action session now receives an empty MCP server map regardless of trust state, while tool-enabled isolated prompts (such as lens refreshes) keep their trust-approved servers, so the "cannot invoke side-effect tools" guarantee holds end to end.
- **Reject shell metacharacters in npm marketplace package and version fields.** The npm-global install spec flows into `npm install -g <package>@<version>`, which is spawned through a shell on Windows. Package and version are now validated against the npm name and exact-version grammar at parse time (mirroring the existing bin-name check), so a malicious enrolled marketplace source cannot inject commands through those fields.
- **Remove the inert marketplace approval and enrollment scaffolding.** The MarketplaceApprovalStore and MarketplaceEnrollmentStore recorded approvals but were never wired into production and never consulted before execution (isApproved had no callers), so they read as a control without being one. The substantive guarantee, sha256 verification of the downloaded artifact before it is written, stays in ToolInstaller. The dead stores and their descriptor-hash helpers are removed.
- **Consolidate filesystem containment on a single realpath-safe primitive.** Routes the working-memory reader (`assertConfined`) and the prompt-library store (`assertPromptsPathConfined`) through the shared `assertContained` guard, which realpath-canonicalizes the root and walks every path component for symlinks, instead of their own lexical `path.resolve` checks. This closes a gap where a symlinked root component could bypass confinement on those two surfaces. Domain-specific error messages are preserved with the original containment error attached as the cause, and symlink-rejection regression tests are added for both.
...
- **Add per-mind execution trust boundary (Stage 1)** — Introduced MindTrustService with atomic JSON ledger. Cron job scheduling and MCP server startup now require an explicit per-mind trust grant. Pending or revoked minds silently skip all workspace execution. Existing locally registered minds receive a legacy trusted grant via idempotent migration. New IPC surface exposes trust status, grant, and revoke without leaking raw MCP configuration.
- **Pin marketplace tool sources to immutable commit SHAs and require explicit operator approval** — Reject mutable Git refs (branches, tags, short SHAs) at enrollment; add --ignore-scripts to npm installs; remove manifest-controlled preflight execution; make startup reconciliation discovery-only; classify legacy mutable-source tools as legacy-unverified; add MarketplaceEnrollmentStore and MarketplaceApprovalStore with atomic writes
- **Bind Canvas actions to renderer user gestures and remove default tools** — Canvas action dispatches now require a renderer gesture grant minted only from a real Approve click. CanvasServer validates the grant before creating any session. Isolated Canvas sessions run with no tools. Legacy sendAction() returns a migration error.
- **Enforce realpath containment and private canvas storage for Lens and Canvas** — Replace lexical isPathInside checks with lstat+realpath containment (assertContained) and atomic writes (writeAtomically) across ViewDiscovery, CanvasService, and CanvasServer. Move generated canvas files to app-owned userData/canvas/ directory. Reject symlinks, junctions, and traversal at every read/write/serve boundary.
- **Contain Canvas dispatch failures and MCP verification teardown** — Guards onActionStatus callback in CanvasServer.publishActionStatus with try/catch and bounded logging; adds terminal catch on void dispatchAction() and void handleRequest() to prevent unhandled promise rejections; wraps MindManager.verifyMcpConfiguration disconnect() in try/finally for explicit disposable-session lifecycle; restricts main.ts canvas action log to bounded fields only (no action.data or secrets)
- **Close final remediation review blockers: cron trust enforcement, MCP revocation, marketplace ref pinning** — Guard all CronService execution paths against untrusted/revoked minds; sever active MCP connections immediately on trust revocation via a reloadMind callback; resolve mutable marketplace source refs to immutable commit SHAs via MarketplaceRefPinner before they reach the enforcement boundary
