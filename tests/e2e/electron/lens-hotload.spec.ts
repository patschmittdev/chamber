...
function writeCanvasLensView(
  ...
  fs.writeFileSync(
    ...
    [
      ...
      '  </main>',
      '  <script>',
      '    window.canvas.onActionStatus(function(status) {',
      '      const labels = { accepted: "Action received", running: "Action in progress", completed: "Action completed", failed: "Action failed" };',
      '      document.getElementById("action-result").textContent = labels[status.status];',
      '    });',
      '    async function sendSmokeAction() {',
      '      document.getElementById("action-result").textContent = "Sending action";',
      '      await window.canvas.sendAction("smoke-test-click", { ok: true });',
      '    }',
      '  </script>',
      '</body>',
      '</html>',
